...
import './Pages.css'
export default function Pages({totalPages,changePage,page,width}) {
  const [iop,setI]=useState(2)
  const getI = (i,totalPages) => {
    if (i<totalPages) {
      setI(i)
    }
    if (i===totalPages) {
  ...
  return (
    <div className='pagination'>
        ...
        <div className={page===1 ? 'firstA' : 'first h'}  onClick={ page===1 ? null : () => {changePage(1);getI(1)}}>
        ...
        <div className={page===1 ? 'previosA' : 'number h'}  onClick={page===1 ? null : () => {changePage(page-1);getI(page-1,totalPages)}}>
        ...
        {pagesArray.map(btn => 
          <div  className={page===btn ? 'number activee h' : 'number h'} key={btn} onClick={() => changePage(btn)}>{btn}</div>
        )}

        <div className={totalPages<=page ? 'previosA' : 'next number h'}   onClick={totalPages<=page ? null : () => {changePage(page+1);getI(page+1,totalPages)}}>
          <svg width="9" height="13" viewBox="0 0 9 13" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className={totalPages<=page ? 'lastA' : 'last h'}  onClick={totalPages<=page ? null : () => {changePage(totalPages);getI(totalPages,totalPages)}}>
          <svg width="14" height="13" viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg">
